...
test(`Verify Product Header and Images for Product ${product.productName}`, {tag: ['@smoke', '@UI']}, async ({AdminhomePage})=> {
        ...
        const productinfopage: ProductsInfoPage = await resultspage.selectProduct(product.productName);
...
test(`Verify Metadata for ${product.productName}`, {tag: ['@regression', '@UI']}, async ({AdminhomePage})=> {
        ...
        const fullProductDetails: Map<string, string|null|number> = await productinfopage.getFullProductDetails(); 
